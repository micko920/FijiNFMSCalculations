...
calcFRLSoftwoodPlantations <- function() {
  ...
  sw$area_planted_ha <- round(sw_hvol_parea[,3])
  mArea <- mean(sw$area_planted_ha)
  
  # Accumulation of C on planted areas over the Reference Period
  sw$cgainp_t <- sw$area_planted_ha * FRLParams$maicp
  # Average annual C accumulation on planted areas over the Reference Period
  mcpp <- mArea * FRLParams$maicp
  
  # Uncertainty analysis (removals in Softwood Plantations)
  resmcpp <- vector() # Vector that collects the results
  ...
  for (i in 1:FRLParams$runs) { # i <- 1
    ...
    swi$cppt <- swi$area_planted_ha * maicpi
    # Total average annual C accumulation
    resmcpp[i] <- mean(swi$cppt)
  }
  ...
  swd_planted_cstock <- growthTotals(
    ...
    growthMatrix(FRLParams$Ty,
                 mArea,
                 yearly_growth,
                 projection=FRLMonitoringPeriodProjectionGrowthLength, offset = FRLMonitoringPeriodProjectionGrowthOffset) * FRLParams$maicp,
  ...
  for (i in 1:FRLParams$runs) { # i <- 1
    vAreas <- rtriangle(
      n = 1,
      theta = sw$area_planted_ha,
      lower = sw$area_planted_ha - (sw$area_planted_ha + 0.0001) * FRLParams$errSwPlantations,
      upper = sw$area_planted_ha + (sw$area_planted_ha + 0.0001) * FRLParams$errSwPlantations
    )
    # Random realization of MAI AGB (25% error)
    maicpi <- rtriangle(
      n = FRLParams$Ty+7,
    ...
    r <- colSums(gm * mean(vAreas * maicpi))
    if (i==1) vswcstock <- r
  ...
  ec_sw_aar <- mcpp * FRLParams$etacc # Estimate
  lci_ec_sw_aar <- quantile(resmcpp * FRLParams$etacc, probs = FRLParams$qlci) # Lower CI limit
  uci_ec_sw_aar <- quantile(resmcpp * FRLParams$etacc, probs = FRLParams$quci) # Upper CI limit
  v_ec_sw_aar <- resmcpp * FRLParams$etacc # MC estimate
...
#' 
#' #' @export
#' OLD___calcFRLSoftwoodPlantations <- function() {
#'   # Volumes extracted from Softwood Plantations
#'   sw <- hwsw_volharv[, c(1, 3)] # Softwood data
#'   names(sw) <- c("year", "vol_m3") # Rename columns
#'   if (debug_frl) {
...
#' 
#'   # Accumulation of C on planted areas over the Reference Period
#'   sw$cgainp_t <- sw$area_planted_ha * FRLParams$deltaT * FRLParams$maicp # MGGG CORRECT THIS
#'   # Average annual C accumulation on planted areas over the Reference Period
