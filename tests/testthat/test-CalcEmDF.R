...
test_that("Baseline FRL Data example", {
  expect_equal(CalcEmDF(2681.642, 199.6785), 535466.2521)
})

test_that("rational number", {
  expect_equal(CalcEmDF(2, (1 / 3)), 0.66666667)
})

test_that("Mult by AD = zero", {
  expect_equal(CalcEmDF(0, 3), 0)
})

test_that("Mult by EF = zero", {
  expect_equal(CalcEmDF(3, 0), 0)
})

test_that("Mult by both zero", {
  expect_equal(CalcEmDF(0, 0), 0)
})

test_that("Mult by negative number", {
  expect_equal(CalcEmDF(-1, 1), -1)
})


test_that("Confirm not equal", {
  expect_false(isTRUE(all.equal(CalcEmDF(1, 2), 20)))
})
